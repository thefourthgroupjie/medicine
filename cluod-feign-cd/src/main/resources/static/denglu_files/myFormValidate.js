...
/**
 *  Ajax通用提交表单
 *  @var  form表单的id属性值  form_id   (注册，登录)
...
function post_form(form_id,subbmit_url){
   ...
   // var loading = $.dialog.tips('数据加载中...', 600000, 'loading.gif');
    //  序列化表单值
    ...
    $.post(subbmit_url,data,function(result){
     ...
     if(result.status == 1){
           if(result.result.return_url){
        	  // layer.alert(result.msg, {icon: 2});// alert(error);
        	   $.dialog.tips(result.msg, 3, 'success.gif', function () {  location.href = result.result.return_url;})
            }else{
                //  刷新页面
            	 $.dialog.tips(result.msg, 3, 'success.gif', function () {location.reload();})
            }
            return;
     }
    })
